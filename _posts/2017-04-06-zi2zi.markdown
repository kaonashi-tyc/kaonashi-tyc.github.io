...
<img src="/assets/cover.png" alt="fail" style="width:100%;"/>
</div>

Generated samples. Related code can be found **[here](https://github.com/kaonashi-tyc/char2char)**
## Motivation
...
* The generated images are often time blurry
* The model fails under more stylized fonts
* Limit to only one style at a time

That is how zi2zi comes to being, another attempt to solve the above problems in one blow, blessed with the mighty power of [GAN](https://arxiv.org/abs/1406.2661). Personally, it also serves as a playground for me to tap into the wonderland of GAN and its variants.

## One GAN to Rule Them All
**TL;DR:** It is a conditional generative adversarial network, combined from 3 awesome papers, plus a little additional spice:
...
The structure of **Encoder**, **Decoder** and **Discriminator** are directly borrowed from the pix2pix model, specifically the Unet model, which are detailed in the original [paper's Appendix section](https://arxiv.org/abs/1611.07004)
### Intuition
One important realization during the conceptualization of this project is the necessity to model **multiple font styles at the same time**. Imagine a human designer work on a new typeface, they are definitely not learning the alphabet from scratch! Real world designers go through years' of training to understand the structure of the letters/characters and the underlying design principles, by studying classic typefaces in history. Thus it is essential to have the model aware of other existing fonts, and benefit from past experience.

### Adding Category Embedding for One-to-Many Modeling
The original pix2pix model doesn't handle one-to-many relation out-of-box, such as in our case, same character could appear in multiple fonts. The introduction of category embedding solves this problem by concatenating a **non-trainable gaussian noise** to the character embedding for each style, an idea stolen from [Google's zero-shot GNMT paper](https://arxiv.org/abs/1611.04558). Also by doing this, encoder can be shared among different fonts, and used in the **fine-tuning** process, which we are going to talk about later. 

### The More Losses, the Better
After introducing category embedding, a new problem emerges: the model starting to confuse and mix multiple font styles together. Inspired from the [AC-GAN model](https://arxiv.org/pdf/1610.09585.pdf), the **multi-class category loss** is added to supervise the discriminator to punish and suppress such cases from happening. Also, the introduction of category loss improves over training stability.

Another important piece of the model is the **constant loss** borrowed from [DTN network](https://arxiv.org/abs/1611.02200). It follows a simple idea: the source and generated character should resemble the same character, thus they must appear close to each other in the embedded space as well. As the with the AC-GAN case, the introduction of this loss **GREATLY, GREATLY** improves the convergence speed, by enforcing the encoder to preserve the identity of generated character, narrowing down the possible search space.
